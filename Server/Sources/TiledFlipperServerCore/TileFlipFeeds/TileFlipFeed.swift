...
//
//  アプリ側 `TiledFlipper/TileFlipFeeds/*.swift` の移植。
//  アプリでは供給元をメインアクターに閉じ込めて内部状態を守っているが、
//  サーバーにはメインアクターが無いので、同じ役目の `TileFlipFeedActor` を置いてある。
//  それ以外の作りはアプリ側と揃えてあるので、演出を直すときは両方を見比べること。
//

/// 差し替え指示を作る側をまとめて閉じ込めるアクター。
///
/// 供給元の内部状態 (カーソルや各タイルへ流した画像) はここでのみ触る。
/// 作る仕事は 1 件ずつが軽く、待ち時間はすべて `Task.sleep` の中断なので、
/// 複数の RPC が同じアクターを共有しても互いを待たせない。
@globalActor
actor TileFlipFeedActor {
    static let shared = TileFlipFeedActor()
}

/// タイル 1 枚を別のアートワークへ差し替える指示。
///
/// 「どの位置を」「どの画像に」だけを持つ。フリップの長さや進行度といった
/// 見た目は受け取る側 (クライアント) の担当なので、指示には含めない。
...
/// 差し替え指示を流し込む供給元。
///
/// 演出を変えるにはこの protocol に適合する型を用意して差し替えればよく、
/// 流す側 (TileFlipService) は変わらない。
///
...
final class RandomWalkTileFlipFeed: TileFlipFeed {
    ...
    /// 選択が次のタイルへ移るまでの間隔。
    /// クライアントのフリップ 1 回分より短くすることで、フリップが完了する前に
    /// 次が始まり、連鎖しているように見える。
    ...
    ///
    /// 受け取り側が読むのをやめる (ストリームがキャンセルされる) と
